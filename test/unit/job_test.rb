...
class JobTest < ActiveSupport::TestCase
  ...
  test "Validate Parameter - initiator - using anon" do
    # Job should have a validate_initiator, this method checks if the initiator exists in the user table
    username = "anon"
    job = Job.new(:initiator => username, :destinations => testVarDestinations, :subject => testVarSubject, :content => testVarContent,  :kickOff => testVarKickOff, :repetition => testVarRepetition, :delay => testVarDelay)
    #assert false, "method not yet ready, implement valid username check (does a username with this name exists in the db?) within job model"
    assert !(job.save()), 'delay required'
  end
  ...
  test "Validate Parameter - subject - subject length > 254" do
    ...
    job = Job.new(:initiator => testVarInitiator, :destinations => testVarDestinations, :subject => subject, :content => testVarContent,  :kickOff => testVarKickOff, :repetition => testVarRepetition, :delay => testVarDelay)
 ...
 test "abc" do
  ...
  test "Validate Parameter - kickOff - time is in the past" do
    ...
    job = Job.new(:initiator => testVarInitiator, :destinations => testVarDestinations, :subject => testVarSubject, :content => testVarContent,  :kickOff => time, :repetition => testVarRepetition, :delay => testVarDelay)
  ...
  test "Validate Parameter - repetition - number <= 0    -> no sense" do
    ...
    job = Job.new(:initiator => testVarInitiator, :destinations => testVarDestinations, :subject => testVarSubject, :content => testVarContent,  :kickOff => testVarKickOff, :repetition => repNumber, :delay => testVarDelay)
